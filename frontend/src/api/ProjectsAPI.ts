...
const API_URL = https://waterproject-pace-backend-czbpgcfwe9e3buf2.eastus-01.azurewebsites.net/api/Water;
...
export const fetchProjects = async(
    ...
    try {
       const categoryParams = selectedCategories.map((cat) => `projectTypes=${encodeURIComponent(cat)}`).join('&');
            ...
            const response = await fetch(`${API_URL}/AllProjects?pageHowMany=${pageSize}&pageNum=${pageNum}${selectedCategories.length ? `&${categoryParams}` : ''}`,
                {
                    credentials: 'include',
                }
            );
...
export const updateProject = async (
    ...
    try {
      ...
      try {
        ...
        return null; // If the server doesn't return JSON, that's okay
      }
    } catch (error) {
